...
require_once '../config/settings.php'; // Koneksi ke Pengaturan CMS
...
// Event Highlight (Berita Terbaru)
try {
...
// Encode JSON untuk Javascript Modal
...
<head>
    ...
    <title>Berita & Kegiatan - Laboratorium MMT POLINEMA</title>
    ...
    <style>
        .hero {
            ...
            /* TINGGI HERO LEBIH PENDEK DARI BERANDA */
            height: 300px !important; 
        }
        /* Penyesuaian font judul agar pas di hero pendek */
...
<body id="top">
                    ...
                    <?php if (count($news_items) > 0): ?>
                        ...
                        <?php if ($total_pages > 1): ?>
                            <div class="pagination">
                                ...
                                if($page > 1) {
                                    $q['page'] = $page - 1;
                                    echo '<a class="page-btn" href="?'.http_build_query($q).'">&laquo;</a>';
                                }
                                ...
                                foreach($pages as $p) {
                                    if($p === '...') {
                                        ...
                                        $q['page'] = $p;
                                        $act = ($p == $page) ? 'active' : '';
                                        echo '<a class="page-btn '.$act.'" href="?'.http_build_query($q).'">'.$p.'</a>';
                                    }
                                }
                                ...
                                if($page < $total_pages) {
                                    $q['page'] = $page + 1;
                                    echo '<a class="page-btn" href="?'.http_build_query($q).'">&raquo;</a>';
                                }
                                ?>
                            </div>
                        <?php endif; ?>
                ...
                <aside class="sidebar">
                    ...
                    <div class="widget widget-calendar">
                        ...
                        <?php
                            ...
                            echo "<a href='".getCalUrl($prevMonth, $prevYear)."' style='text-decoration:none; font-weight:bold; font-size:18px; color:#003b8e;'>&laquo;</a>";
                            echo "<span style='font-weight:bold; color:#003b8e;'>$monthName $cYear</span>";
                            echo "<a href='".getCalUrl($nextMonth, $nextYear)."' style='text-decoration:none; font-weight:bold; font-size:18px; color:#003b8e;'>&raquo;</a>";
                            echo "</div>";
                            ...
                            for ($day = 1; $day <= $daysInMonth; $day++) {
                                $dateStr = sprintf('%04d-%02d-%02d', $cYear, $cMonth, $day);
                                $hasEvent = isset($events_by_date[$dateStr]);
                                $cls = $hasEvent ? 'event-day' : '';
                                $data = $hasEvent ? "data-date='$dateStr'" : "";
                                
                                // Tandai hari ini (border kuning)
                                $todayStyle = ($dateStr == date('Y-m-d')) ? "style='border: 2px solid #ffc700; border-radius:50%; font-weight:bold;'" : "";

                                echo "<td class='$cls' $data $todayStyle>$day</td>";
                    ...
                    <div class="widget widget-news">
                        ...
                        <ul>
                            ...
                            ?>
                            <li>
                                <a href="menu-detail-berita/detail-berita.php?slug=<?= $s['slug'] ?>">
                                    <h4><?= htmlspecialchars($s['title']) ?></h4>
                                    <span class="date"><?= date('d M Y', strtotime($s['created_at'])) ?></span>
                                </a>
                            </li>
                            <?php endwhile; ?>
                        </ul>
    ...
    <div id="modalBackdrop" class="modal-backdrop">
        <div class="modal">
            ...
            <h3>Agenda <span id="modalDate"></span></h3>
    ...
    <script>
        ...
        function openModalForDate(dateStr) {
            ...
            modalDateEl.textContent = dateObj.toLocaleDateString('id-ID', options);
            modalContent.innerHTML = '';

            const events = eventsByDate[dateStr] || [];
            
            if (events.length === 0) {
                modalContent.innerHTML = '<p style="text-align:center; color:#666;">Tidak ada kegiatan pada tanggal ini.</p>';
            } else {
                events.forEach(ev => {
